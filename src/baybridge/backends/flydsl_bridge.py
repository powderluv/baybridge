...
class FlyDslBridge:
    ...
    def render_exec_python(self, ir: PortableKernelIR, target: AMDTarget, match: FlyDslMatch, *, backend_name: str) -> str:
        ...
        return (
            ...
            "_baybridge_adapted_tensor_cache = {}\n\n"
            "_baybridge_default_stream = fx.Stream(None)\n\n"
            ...
            "        cached = _baybridge_adapted_tensor_cache.get(id(value))\n"
            "        if cached is not None:\n"
            "            value_ref, adapted = cached\n"
            "            if value_ref() is value:\n"
            "                return adapted\n"
            "        adapted = flyc.from_dlpack(value)\n"
            "        try:\n"
            "            _baybridge_adapted_tensor_cache[id(value)] = (weakref.ref(value), adapted)\n"
            "        except TypeError:\n"
            "            pass\n"
            "        return adapted\n"
            ...
            f"{kernel_body}\n\n"
            f"@flyc.jit\n"
            f"def {ir.name}_jit({kernel_args}, stream: fx.Stream = _baybridge_default_stream):\n"
            ...
            + f"    return {ir.name}_jit({args}, stream=stream if stream is not None else _baybridge_default_stream)\n"
        )
