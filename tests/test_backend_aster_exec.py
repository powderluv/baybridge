import os
from pathlib import Path

import pytest

import baybridge as bb
from baybridge.backends.aster_exec import AsterExecBackend


@bb.kernel
def aster_exec_copy_kernel(
    src: bb.TensorSpec(shape=(16,), dtype="f32"),
    dst: bb.TensorSpec(shape=(16,), dtype="f32"),
):
    bb.copy(src, dst)


@bb.kernel
def aster_exec_copy_tail_kernel(
    src: bb.TensorSpec(shape=(10,), dtype="f32"),
    dst: bb.TensorSpec(shape=(10,), dtype="f32"),
):
    bb.copy(src, dst)


@bb.kernel
def aster_exec_copy_i32_kernel(
    src: bb.TensorSpec(shape=(16,), dtype="i32"),
    dst: bb.TensorSpec(shape=(16,), dtype="i32"),
):
    bb.copy(src, dst)


@bb.kernel
def aster_exec_add_tail_kernel(
    src: bb.TensorSpec(shape=(2, 5), dtype="f32"),
    other: bb.TensorSpec(shape=(2, 5), dtype="f32"),
    dst: bb.TensorSpec(shape=(2, 5), dtype="f32"),
):
    dst.store(src.load() + other.load())


@bb.kernel
def aster_exec_sub_tail_kernel(
    src: bb.TensorSpec(shape=(2, 5), dtype="f32"),
    other: bb.TensorSpec(shape=(2, 5), dtype="f32"),
    dst: bb.TensorSpec(shape=(2, 5), dtype="f32"),
):
    dst.store(src.load() - other.load())


@bb.kernel
def aster_exec_mul_tail_kernel(
    src: bb.TensorSpec(shape=(2, 5), dtype="f32"),
    other: bb.TensorSpec(shape=(2, 5), dtype="f32"),
    dst: bb.TensorSpec(shape=(2, 5), dtype="f32"),
):
    dst.store(src.load() * other.load())


@bb.kernel
def aster_exec_div_tail_kernel(
    src: bb.TensorSpec(shape=(2, 5), dtype="f32"),
    other: bb.TensorSpec(shape=(2, 5), dtype="f32"),
    dst: bb.TensorSpec(shape=(2, 5), dtype="f32"),
):
    dst.store(src.load() / other.load())


@bb.kernel
def aster_exec_add_i32_kernel(
    src: bb.TensorSpec(shape=(16,), dtype="i32"),
    other: bb.TensorSpec(shape=(16,), dtype="i32"),
    dst: bb.TensorSpec(shape=(16,), dtype="i32"),
):
    dst.store(src.load() + other.load())


@bb.kernel
def aster_exec_add_i32_tail_kernel(
    src: bb.TensorSpec(shape=(10,), dtype="i32"),
    other: bb.TensorSpec(shape=(10,), dtype="i32"),
    dst: bb.TensorSpec(shape=(10,), dtype="i32"),
):
    dst.store(src.load() + other.load())


@bb.kernel
def aster_exec_sub_i32_tail_kernel(
    src: bb.TensorSpec(shape=(10,), dtype="i32"),
    other: bb.TensorSpec(shape=(10,), dtype="i32"),
    dst: bb.TensorSpec(shape=(10,), dtype="i32"),
):
    dst.store(src.load() - other.load())


@bb.kernel
def aster_exec_mul_i32_tail_kernel(
    src: bb.TensorSpec(shape=(10,), dtype="i32"),
    other: bb.TensorSpec(shape=(10,), dtype="i32"),
    dst: bb.TensorSpec(shape=(10,), dtype="i32"),
):
    dst.store(src.load() * other.load())


@bb.kernel
def aster_exec_div_i32_tail_kernel(
    src: bb.TensorSpec(shape=(10,), dtype="i32"),
    other: bb.TensorSpec(shape=(10,), dtype="i32"),
    dst: bb.TensorSpec(shape=(10,), dtype="i32"),
):
    dst.store(src.load() / other.load())


@bb.kernel
def aster_exec_sub_i32_kernel(
    src: bb.TensorSpec(shape=(16,), dtype="i32"),
    other: bb.TensorSpec(shape=(16,), dtype="i32"),
    dst: bb.TensorSpec(shape=(16,), dtype="i32"),
):
    dst.store(src.load() - other.load())


@bb.kernel
def aster_exec_mul_i32_kernel(
    src: bb.TensorSpec(shape=(16,), dtype="i32"),
    other: bb.TensorSpec(shape=(16,), dtype="i32"),
    dst: bb.TensorSpec(shape=(16,), dtype="i32"),
):
    dst.store(src.load() * other.load())


@bb.kernel
def aster_exec_div_i32_kernel(
    src: bb.TensorSpec(shape=(16,), dtype="i32"),
    other: bb.TensorSpec(shape=(16,), dtype="i32"),
    dst: bb.TensorSpec(shape=(16,), dtype="i32"),
):
    dst.store(src.load() / other.load())


@bb.kernel
def aster_exec_copy_f16_kernel(
    src: bb.TensorSpec(shape=(16,), dtype="f16"),
    dst: bb.TensorSpec(shape=(16,), dtype="f16"),
):
    bb.copy(src, dst)


@bb.kernel
def aster_exec_copy_f16_tail_kernel(
    src: bb.TensorSpec(shape=(10,), dtype="f16"),
    dst: bb.TensorSpec(shape=(10,), dtype="f16"),
):
    bb.copy(src, dst)


@bb.kernel
def aster_exec_copy_2d_kernel(
    src: bb.TensorSpec(shape=(4, 4), dtype="f32"),
    dst: bb.TensorSpec(shape=(4, 4), dtype="f32"),
):
    bb.copy(src, dst)


@bb.kernel
def aster_exec_copy_2d_f16_kernel(
    src: bb.TensorSpec(shape=(4, 4), dtype="f16"),
    dst: bb.TensorSpec(shape=(4, 4), dtype="f16"),
):
    bb.copy(src, dst)


@bb.kernel
def aster_exec_copy_2d_f16_tail_kernel(
    src: bb.TensorSpec(shape=(2, 5), dtype="f16"),
    dst: bb.TensorSpec(shape=(2, 5), dtype="f16"),
):
    bb.copy(src, dst)


@bb.kernel
def aster_exec_mfma_gemm_kernel(
    a: bb.TensorSpec(shape=(16, 16), dtype="f16"),
    b: bb.TensorSpec(shape=(16, 16), dtype="f16"),
    c: bb.TensorSpec(shape=(16, 16), dtype="f32"),
):
    bb.gemm(a, b, c)


@bb.kernel
def aster_exec_mfma_bf16_gemm_kernel(
    a: bb.TensorSpec(shape=(16, 16), dtype="bf16"),
    b: bb.TensorSpec(shape=(16, 16), dtype="bf16"),
    c: bb.TensorSpec(shape=(16, 16), dtype="f32"),
):
    bb.gemm(a, b, c)


@bb.kernel
def aster_exec_mfma_fp8_gemm_kernel(
    a: bb.TensorSpec(shape=(16, 32), dtype="fp8"),
    b: bb.TensorSpec(shape=(32, 16), dtype="fp8"),
    c: bb.TensorSpec(shape=(16, 16), dtype="f32"),
):
    bb.gemm(a, b, c)


@bb.kernel
def aster_exec_mfma_bf8_gemm_kernel(
    a: bb.TensorSpec(shape=(16, 32), dtype="bf8"),
    b: bb.TensorSpec(shape=(32, 16), dtype="bf8"),
    c: bb.TensorSpec(shape=(16, 16), dtype="f32"),
):
    bb.gemm(a, b, c)


@bb.kernel
def aster_exec_mfma_fp8_bf8_gemm_kernel(
    a: bb.TensorSpec(shape=(16, 32), dtype="fp8"),
    b: bb.TensorSpec(shape=(32, 16), dtype="bf8"),
    c: bb.TensorSpec(shape=(16, 16), dtype="f32"),
):
    bb.gemm(a, b, c)


@bb.kernel
def aster_exec_mfma_bf8_fp8_gemm_kernel(
    a: bb.TensorSpec(shape=(16, 32), dtype="bf8"),
    b: bb.TensorSpec(shape=(32, 16), dtype="fp8"),
    c: bb.TensorSpec(shape=(16, 16), dtype="f32"),
):
    bb.gemm(a, b, c)


@bb.kernel
def aster_exec_fragment_mfma_kernel(
    a: bb.TensorSpec(shape=(16, 16), dtype="f16"),
    b: bb.TensorSpec(shape=(16, 16), dtype="f16"),
    c: bb.TensorSpec(shape=(16, 16), dtype="f32"),
):
    a_frag = bb.make_fragment_a(a, tile=(16, 16, 16))
    b_frag = bb.make_fragment_b(b, tile=(16, 16, 16))
    acc = bb.mma(a_frag, b_frag, tile=(16, 16, 16), accumulator_dtype="f32")
    bb.copy(acc, c)


@bb.kernel
def aster_exec_fragment_mfma_bf16_kernel(
    a: bb.TensorSpec(shape=(16, 16), dtype="bf16"),
    b: bb.TensorSpec(shape=(16, 16), dtype="bf16"),
    c: bb.TensorSpec(shape=(16, 16), dtype="f32"),
):
    a_frag = bb.make_fragment_a(a, tile=(16, 16, 16))
    b_frag = bb.make_fragment_b(b, tile=(16, 16, 16))
    acc = bb.mma(a_frag, b_frag, tile=(16, 16, 16), accumulator_dtype="f32")
    bb.copy(acc, c)


@bb.kernel
def aster_exec_tcgen05_fragment_mfma_kernel(
    a: bb.TensorSpec(shape=(16, 16), dtype="f16"),
    b: bb.TensorSpec(shape=(16, 16), dtype="f16"),
    c: bb.TensorSpec(shape=(16, 16), dtype="f32"),
):
    tiled_mma = bb.make_tiled_mma(
        bb.nvgpu.tcgen05.MmaF16BF16Op(
            "f16",
            "f32",
            (16, 16, 16),
            bb.nvgpu.tcgen05.CtaGroup.ONE,
            bb.nvgpu.tcgen05.OperandSource.TMEM,
            bb.nvgpu.tcgen05.OperandMajorMode.K,
            bb.nvgpu.tcgen05.OperandMajorMode.N,
        )
    )
    a_frag = tiled_mma.partition_A(a)
    b_frag = tiled_mma.partition_B(b)
    acc = tiled_mma.make_fragment_C((16, 16))
    bb.mma(a_frag, b_frag, c=acc, tile=(16, 16, 16), accumulator_dtype="f32")
    bb.copy(acc, c)


@bb.kernel
def aster_exec_warp_fragment_mfma_bf16_kernel(
    a: bb.TensorSpec(shape=(16, 16), dtype="bf16"),
    b: bb.TensorSpec(shape=(16, 16), dtype="bf16"),
    c: bb.TensorSpec(shape=(16, 16), dtype="f32"),
):
    tiled_mma = bb.make_tiled_mma(
        bb.nvgpu.warp.MmaF16BF16Op("bf16", "f32", (16, 16, 16))
    )
    a_frag = tiled_mma.partition_A(a)
    b_frag = tiled_mma.partition_B(b)
    acc = tiled_mma.make_fragment_C((16, 16))
    bb.mma(a_frag, b_frag, c=acc, tile=(16, 16, 16), accumulator_dtype="f32")
    bb.copy(acc, c)


@bb.kernel
def aster_exec_add_kernel(
    src: bb.TensorSpec(shape=(16,), dtype="f32"),
    other: bb.TensorSpec(shape=(16,), dtype="f32"),
    dst: bb.TensorSpec(shape=(16,), dtype="f32"),
):
    dst.store(src.load() + other.load())


@bb.kernel
def aster_exec_add_broadcast_kernel(
    src: bb.TensorSpec(shape=(10,), dtype="f32"),
    other: bb.TensorSpec(shape=(1,), dtype="f32"),
    dst: bb.TensorSpec(shape=(10,), dtype="f32"),
):
    dst.store(src.load() + other.load().broadcast_to((10,)))


@bb.kernel
def aster_exec_add_broadcast_aligned_kernel(
    src: bb.TensorSpec(shape=(16,), dtype="f32"),
    other: bb.TensorSpec(shape=(1,), dtype="f32"),
    dst: bb.TensorSpec(shape=(16,), dtype="f32"),
):
    dst.store(src.load() + other.load().broadcast_to((16,)))


@bb.kernel
def aster_exec_sub_broadcast_kernel(
    src: bb.TensorSpec(shape=(10,), dtype="f32"),
    other: bb.TensorSpec(shape=(1,), dtype="f32"),
    dst: bb.TensorSpec(shape=(10,), dtype="f32"),
):
    dst.store(src.load() - other.load().broadcast_to((10,)))


@bb.kernel
def aster_exec_mul_broadcast_kernel(
    src: bb.TensorSpec(shape=(10,), dtype="f32"),
    other: bb.TensorSpec(shape=(1,), dtype="f32"),
    dst: bb.TensorSpec(shape=(10,), dtype="f32"),
):
    dst.store(src.load() * other.load().broadcast_to((10,)))


@bb.kernel
def aster_exec_div_broadcast_kernel(
    src: bb.TensorSpec(shape=(10,), dtype="f32"),
    other: bb.TensorSpec(shape=(1,), dtype="f32"),
    dst: bb.TensorSpec(shape=(10,), dtype="f32"),
):
    dst.store(src.load() / other.load().broadcast_to((10,)))


@bb.kernel
def aster_exec_add_2d_broadcast_kernel(
    src: bb.TensorSpec(shape=(2, 5), dtype="f32"),
    other: bb.TensorSpec(shape=(1, 1), dtype="f32"),
    dst: bb.TensorSpec(shape=(2, 5), dtype="f32"),
):
    dst.store(src.load() + other.load().broadcast_to((2, 5)))


@bb.kernel
def aster_exec_sub_lhs_broadcast_kernel(
    src: bb.TensorSpec(shape=(1,), dtype="f32"),
    other: bb.TensorSpec(shape=(10,), dtype="f32"),
    dst: bb.TensorSpec(shape=(10,), dtype="f32"),
):
    dst.store(src.load().broadcast_to((10,)) - other.load())


@bb.kernel
def aster_exec_div_lhs_broadcast_kernel(
    src: bb.TensorSpec(shape=(1,), dtype="f32"),
    other: bb.TensorSpec(shape=(10,), dtype="f32"),
    dst: bb.TensorSpec(shape=(10,), dtype="f32"),
):
    dst.store(src.load().broadcast_to((10,)) / other.load())


@bb.kernel
def aster_exec_sub_2d_lhs_broadcast_kernel(
    src: bb.TensorSpec(shape=(1, 1), dtype="f32"),
    other: bb.TensorSpec(shape=(2, 5), dtype="f32"),
    dst: bb.TensorSpec(shape=(2, 5), dtype="f32"),
):
    dst.store(src.load().broadcast_to((2, 5)) - other.load())


@bb.kernel
def aster_exec_add_2d_kernel(
    src: bb.TensorSpec(shape=(4, 4), dtype="f32"),
    other: bb.TensorSpec(shape=(4, 4), dtype="f32"),
    dst: bb.TensorSpec(shape=(4, 4), dtype="f32"),
):
    dst.store(src.load() + other.load())


@bb.kernel
def aster_exec_sub_2d_kernel(
    src: bb.TensorSpec(shape=(4, 4), dtype="f32"),
    other: bb.TensorSpec(shape=(4, 4), dtype="f32"),
    dst: bb.TensorSpec(shape=(4, 4), dtype="f32"),
):
    dst.store(src.load() - other.load())


@bb.kernel
def aster_exec_mul_2d_kernel(
    src: bb.TensorSpec(shape=(4, 4), dtype="f32"),
    other: bb.TensorSpec(shape=(4, 4), dtype="f32"),
    dst: bb.TensorSpec(shape=(4, 4), dtype="f32"),
):
    dst.store(src.load() * other.load())


@bb.kernel
def aster_exec_div_2d_kernel(
    src: bb.TensorSpec(shape=(4, 4), dtype="f32"),
    other: bb.TensorSpec(shape=(4, 4), dtype="f32"),
    dst: bb.TensorSpec(shape=(4, 4), dtype="f32"),
):
    dst.store(src.load() / other.load())


@bb.kernel
def aster_exec_add_2d_i32_kernel(
    src: bb.TensorSpec(shape=(4, 4), dtype="i32"),
    other: bb.TensorSpec(shape=(4, 4), dtype="i32"),
    dst: bb.TensorSpec(shape=(4, 4), dtype="i32"),
):
    dst.store(src.load() + other.load())


@bb.kernel
def aster_exec_add_i32_broadcast_kernel(
    src: bb.TensorSpec(shape=(10,), dtype="i32"),
    other: bb.TensorSpec(shape=(1,), dtype="i32"),
    dst: bb.TensorSpec(shape=(10,), dtype="i32"),
):
    dst.store(src.load() + other.load().broadcast_to((10,)))


@bb.kernel
def aster_exec_add_i32_broadcast_aligned_kernel(
    src: bb.TensorSpec(shape=(16,), dtype="i32"),
    other: bb.TensorSpec(shape=(1,), dtype="i32"),
    dst: bb.TensorSpec(shape=(16,), dtype="i32"),
):
    dst.store(src.load() + other.load().broadcast_to((16,)))


@bb.kernel
def aster_exec_sub_i32_broadcast_kernel(
    src: bb.TensorSpec(shape=(10,), dtype="i32"),
    other: bb.TensorSpec(shape=(1,), dtype="i32"),
    dst: bb.TensorSpec(shape=(10,), dtype="i32"),
):
    dst.store(src.load() - other.load().broadcast_to((10,)))


@bb.kernel
def aster_exec_mul_i32_broadcast_kernel(
    src: bb.TensorSpec(shape=(10,), dtype="i32"),
    other: bb.TensorSpec(shape=(1,), dtype="i32"),
    dst: bb.TensorSpec(shape=(10,), dtype="i32"),
):
    dst.store(src.load() * other.load().broadcast_to((10,)))


@bb.kernel
def aster_exec_div_i32_broadcast_kernel(
    src: bb.TensorSpec(shape=(10,), dtype="i32"),
    other: bb.TensorSpec(shape=(1,), dtype="i32"),
    dst: bb.TensorSpec(shape=(10,), dtype="i32"),
):
    dst.store(src.load() / other.load().broadcast_to((10,)))


@bb.kernel
def aster_exec_add_2d_i32_broadcast_kernel(
    src: bb.TensorSpec(shape=(2, 5), dtype="i32"),
    other: bb.TensorSpec(shape=(1, 1), dtype="i32"),
    dst: bb.TensorSpec(shape=(2, 5), dtype="i32"),
):
    dst.store(src.load() + other.load().broadcast_to((2, 5)))


@bb.kernel
def aster_exec_sub_i32_lhs_broadcast_kernel(
    src: bb.TensorSpec(shape=(1,), dtype="i32"),
    other: bb.TensorSpec(shape=(10,), dtype="i32"),
    dst: bb.TensorSpec(shape=(10,), dtype="i32"),
):
    dst.store(src.load().broadcast_to((10,)) - other.load())


@bb.kernel
def aster_exec_div_i32_lhs_broadcast_kernel(
    src: bb.TensorSpec(shape=(1,), dtype="i32"),
    other: bb.TensorSpec(shape=(10,), dtype="i32"),
    dst: bb.TensorSpec(shape=(10,), dtype="i32"),
):
    dst.store(src.load().broadcast_to((10,)) / other.load())


@bb.kernel
def aster_exec_sub_2d_i32_lhs_broadcast_kernel(
    src: bb.TensorSpec(shape=(1, 1), dtype="i32"),
    other: bb.TensorSpec(shape=(2, 5), dtype="i32"),
    dst: bb.TensorSpec(shape=(2, 5), dtype="i32"),
):
    dst.store(src.load().broadcast_to((2, 5)) - other.load())


@bb.kernel
def aster_exec_sub_2d_i32_kernel(
    src: bb.TensorSpec(shape=(4, 4), dtype="i32"),
    other: bb.TensorSpec(shape=(4, 4), dtype="i32"),
    dst: bb.TensorSpec(shape=(4, 4), dtype="i32"),
):
    dst.store(src.load() - other.load())


@bb.kernel
def aster_exec_mul_2d_i32_kernel(
    src: bb.TensorSpec(shape=(4, 4), dtype="i32"),
    other: bb.TensorSpec(shape=(4, 4), dtype="i32"),
    dst: bb.TensorSpec(shape=(4, 4), dtype="i32"),
):
    dst.store(src.load() * other.load())


@bb.kernel
def aster_exec_div_2d_i32_kernel(
    src: bb.TensorSpec(shape=(4, 4), dtype="i32"),
    other: bb.TensorSpec(shape=(4, 4), dtype="i32"),
    dst: bb.TensorSpec(shape=(4, 4), dtype="i32"),
):
    dst.store(src.load() / other.load())


@bb.kernel
def aster_exec_sub_kernel(
    src: bb.TensorSpec(shape=(16,), dtype="f32"),
    other: bb.TensorSpec(shape=(16,), dtype="f32"),
    dst: bb.TensorSpec(shape=(16,), dtype="f32"),
):
    dst.store(src.load() - other.load())


@bb.kernel
def aster_exec_mul_kernel(
    src: bb.TensorSpec(shape=(16,), dtype="f32"),
    other: bb.TensorSpec(shape=(16,), dtype="f32"),
    dst: bb.TensorSpec(shape=(16,), dtype="f32"),
):
    dst.store(src.load() * other.load())


@bb.kernel
def aster_exec_div_kernel(
    src: bb.TensorSpec(shape=(16,), dtype="f32"),
    other: bb.TensorSpec(shape=(16,), dtype="f32"),
    dst: bb.TensorSpec(shape=(16,), dtype="f32"),
):
    dst.store(src.load() / other.load())


@bb.kernel
def aster_exec_unsupported_pipeline_kernel(
    src: bb.TensorSpec(shape=(16,), dtype="f32"),
    other: bb.TensorSpec(shape=(16,), dtype="f32"),
    dst: bb.TensorSpec(shape=(16,), dtype="f32"),
):
    bb.copy(src, dst)
    dst.store(src.load() + other.load())


def _install_fake_aster(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    aster_root = tmp_path / "aster-root"
    package_root = tmp_path / "fake-site"
    package_dir = package_root / "aster"
    testing_dir = package_dir / "testing"
    build_bin = aster_root / ".aster-baybridge" / "bin"
    library_dir = aster_root / "mlir_kernels" / "library" / "common"

    build_bin.mkdir(parents=True, exist_ok=True)
    library_dir.mkdir(parents=True, exist_ok=True)
    testing_dir.mkdir(parents=True, exist_ok=True)

    for name in ("aster-opt", "aster-translate"):
        tool = build_bin / name
        tool.write_text("#!/bin/sh\nexit 0\n", encoding="utf-8")
        tool.chmod(0o755)

    for name in ("register-init.mlir", "indexing.mlir", "copies.mlir"):
        (library_dir / name).write_text("// fake library\n", encoding="utf-8")

    (package_dir / "__init__.py").write_text("", encoding="utf-8")
    (package_dir / "ir.py").write_text(
        "class Context:\n"
        "    def __enter__(self):\n"
        "        return self\n"
        "    def __exit__(self, exc_type, exc, tb):\n"
        "        return False\n",
        encoding="utf-8",
    )
    (package_dir / "pass_pipelines.py").write_text(
        "def get_pass_pipeline(name):\n"
        "    return name\n",
        encoding="utf-8",
    )
    (package_dir / "utils.py").write_text(
        "from pathlib import Path\n"
        "def assemble_to_hsaco(asm, target='gfx942', wavefront_size=64, output_path=None):\n"
        "    path = Path(output_path)\n"
        "    path.write_bytes(b'fake-hsaco')\n"
        "    return str(path)\n",
        encoding="utf-8",
    )
    (package_dir / "hip.py").write_text(
        "def _apply(lhs, rhs, op):\n"
        "    if isinstance(lhs, (list, tuple)):\n"
        "        if isinstance(rhs, (list, tuple)) and len(lhs) == 1 and len(rhs) != 1:\n"
        "            return [_apply(lhs[0], right, op) for right in rhs]\n"
        "        if isinstance(rhs, (list, tuple)) and len(rhs) == 1:\n"
        "            return [_apply(left, rhs[0], op) for left in lhs]\n"
        "        if not isinstance(rhs, (list, tuple)):\n"
        "            return [_apply(left, rhs, op) for left in lhs]\n"
        "        return [_apply(left, right, op) for left, right in zip(lhs, rhs)]\n"
        "    if op == 'add':\n"
        "        return lhs + rhs\n"
        "    if op == 'sub':\n"
        "        return lhs - rhs\n"
        "    if op == 'mul':\n"
        "        return lhs * rhs\n"
        "    if isinstance(lhs, int) and isinstance(rhs, int):\n"
        "        return lhs // rhs\n"
        "    return lhs / rhs\n"
        "def _decode_fp8_scalar(raw):\n"
        "    sign = -1.0 if (raw & 0x80) else 1.0\n"
        "    exponent = (raw >> 3) & 0xF\n"
        "    mantissa = raw & 0x7\n"
        "    if exponent == 0:\n"
        "        return sign * ((mantissa / 8.0) * (2.0 ** -7))\n"
        "    return sign * ((1.0 + mantissa / 8.0) * (2.0 ** (exponent - 8)))\n"
        "def _decode_bf8_scalar(raw):\n"
        "    sign = -1.0 if (raw & 0x80) else 1.0\n"
        "    exponent = (raw >> 2) & 0x1F\n"
        "    mantissa = raw & 0x3\n"
        "    if exponent == 0:\n"
        "        return sign * ((mantissa / 4.0) * (2.0 ** -15))\n"
        "    return sign * ((1.0 + mantissa / 4.0) * (2.0 ** (exponent - 16)))\n"
        "def _decode_float8_list(value, scalar_decoder):\n"
        "    if isinstance(value, list):\n"
        "        if not value:\n"
        "            return value\n"
        "        if isinstance(value[0], list):\n"
        "            if not value[0] or not isinstance(value[0][0], int):\n"
        "                return value\n"
        "            return [[scalar_decoder(int(item)) for item in row] for row in value]\n"
        "        if not isinstance(value[0], int):\n"
        "            return value\n"
        "        return [scalar_decoder(int(item)) for item in value]\n"
        "    return None\n"
        "def _decode_fp8(value):\n"
        "    decoded = _decode_float8_list(value, _decode_fp8_scalar)\n"
        "    if decoded is not None:\n"
        "        return decoded\n"
        "    if not hasattr(value, 'dtype') or getattr(value.dtype, 'kind', '') != 'u' or getattr(value.dtype, 'itemsize', 0) != 1:\n"
        "        return value\n"
        "    import numpy as np\n"
        "    raw = value.astype(np.uint8)\n"
        "    sign = np.where((raw & 0x80) != 0, -1.0, 1.0).astype(np.float32)\n"
        "    exponent = ((raw >> 3) & 0xF).astype(np.int32)\n"
        "    mantissa = (raw & 0x7).astype(np.float32)\n"
        "    normal = np.ldexp(1.0 + mantissa / 8.0, exponent - 8)\n"
        "    subnormal = np.ldexp(mantissa / 8.0, -7)\n"
        "    return sign * np.where(exponent == 0, subnormal, normal)\n"
        "def _decode_bf8(value):\n"
        "    decoded = _decode_float8_list(value, _decode_bf8_scalar)\n"
        "    if decoded is not None:\n"
        "        return decoded\n"
        "    if not hasattr(value, 'dtype') or getattr(value.dtype, 'kind', '') != 'u' or getattr(value.dtype, 'itemsize', 0) != 1:\n"
        "        return value\n"
        "    import numpy as np\n"
        "    raw = value.astype(np.uint8)\n"
        "    sign = np.where((raw & 0x80) != 0, -1.0, 1.0).astype(np.float32)\n"
        "    exponent = ((raw >> 2) & 0x1F).astype(np.int32)\n"
        "    mantissa = (raw & 0x3).astype(np.float32)\n"
        "    normal = np.ldexp(1.0 + mantissa / 4.0, exponent - 16)\n"
        "    subnormal = np.ldexp(mantissa / 4.0, -15)\n"
        "    return sign * np.where(exponent == 0, subnormal, normal)\n"
        "def _decode_bf16(value):\n"
        "    if not hasattr(value, 'dtype') or getattr(value.dtype, 'kind', '') != 'u' or getattr(value.dtype, 'itemsize', 0) != 2:\n"
        "        return value\n"
        "    import numpy as np\n"
        "    return (value.astype(np.uint32) << 16).view(np.float32)\n"
        "def _matmul(lhs, rhs, lhs_dtype=None, rhs_dtype=None):\n"
        "    if lhs_dtype == 'fp8':\n"
        "        lhs = _decode_fp8(lhs)\n"
        "    elif lhs_dtype == 'bf8':\n"
        "        lhs = _decode_bf8(lhs)\n"
        "    else:\n"
        "        lhs = _decode_bf16(lhs)\n"
        "    if rhs_dtype == 'fp8':\n"
        "        rhs = _decode_fp8(rhs)\n"
        "    elif rhs_dtype == 'bf8':\n"
        "        rhs = _decode_bf8(rhs)\n"
        "    else:\n"
        "        rhs = _decode_bf16(rhs)\n"
        "    if hasattr(lhs, '__matmul__'):\n"
        "        return lhs @ rhs\n"
        "    rows = len(lhs)\n"
        "    inner = len(lhs[0])\n"
        "    cols = len(rhs[0])\n"
        "    return [[sum(lhs[row][kk] * rhs[kk][col] for kk in range(inner)) for col in range(cols)] for row in range(rows)]\n"
        "def execute_hsaco(hsaco_path, kernel_name, input_arrays, output_arrays, grid_dim=(1,1,1), block_dim=(1,1,1), num_iterations=1):\n"
        "    if 'gemm' in kernel_name or 'mfma' in kernel_name:\n"
        "        lhs_dtype = None\n"
        "        rhs_dtype = None\n"
        "        if 'fp8_bf8' in kernel_name:\n"
        "            lhs_dtype = 'fp8'\n"
        "            rhs_dtype = 'bf8'\n"
        "        elif 'bf8_fp8' in kernel_name:\n"
        "            lhs_dtype = 'bf8'\n"
        "            rhs_dtype = 'fp8'\n"
        "        elif 'fp8' in kernel_name:\n"
        "            lhs_dtype = 'fp8'\n"
        "            rhs_dtype = 'fp8'\n"
        "        elif 'bf8' in kernel_name:\n"
        "            lhs_dtype = 'bf8'\n"
        "            rhs_dtype = 'bf8'\n"
        "        output_arrays[0][:] = _matmul(input_arrays[0], input_arrays[1], lhs_dtype, rhs_dtype)\n"
        "    elif len(input_arrays) == 2:\n"
        "        is_array_like = all(hasattr(value, 'shape') or hasattr(value, 'tolist') for value in input_arrays)\n"
        "        is_integer_array = bool(is_array_like and getattr(input_arrays[0], 'dtype', None) is not None and getattr(input_arrays[0].dtype, 'kind', '') in ('i', 'u'))\n"
        "        if 'sub' in kernel_name:\n"
        "            if is_array_like:\n"
        "                output_arrays[0][:] = input_arrays[0] - input_arrays[1]\n"
        "            else:\n"
        "                output_arrays[0][:] = _apply(input_arrays[0], input_arrays[1], 'sub')\n"
        "        elif 'mul' in kernel_name:\n"
        "            if is_array_like:\n"
        "                output_arrays[0][:] = input_arrays[0] * input_arrays[1]\n"
        "            else:\n"
        "                output_arrays[0][:] = _apply(input_arrays[0], input_arrays[1], 'mul')\n"
        "        elif 'div' in kernel_name:\n"
        "            if is_array_like:\n"
        "                output_arrays[0][:] = input_arrays[0] // input_arrays[1] if is_integer_array else input_arrays[0] / input_arrays[1]\n"
        "            else:\n"
        "                output_arrays[0][:] = _apply(input_arrays[0], input_arrays[1], 'div')\n"
        "        elif is_array_like:\n"
        "            output_arrays[0][:] = input_arrays[0] + input_arrays[1]\n"
        "        else:\n"
        "            output_arrays[0][:] = _apply(input_arrays[0], input_arrays[1], 'add')\n"
        "    else:\n"
        "        output_arrays[0][:] = input_arrays[0]\n"
        "    return [1234]\n",
        encoding="utf-8",
    )
    (testing_dir / "__init__.py").write_text(
        "from contextlib import contextmanager\n"
        "def compile_mlir_file_to_asm(mlir_file, kernel_name, pass_pipeline, ctx, preprocess=None, library_paths=None, **kwargs):\n"
        "    text = open(mlir_file, 'r', encoding='utf-8').read()\n"
        "    if preprocess is not None:\n"
        "        text = preprocess(text)\n"
        "    return f'// fake asm for {kernel_name}\\n' + text, object()\n"
        "@contextmanager\n"
        "def hsaco_file(path):\n"
        "    yield path\n",
        encoding="utf-8",
    )

    monkeypatch.syspath_prepend(str(package_root))
    monkeypatch.setenv("BAYBRIDGE_ASTER_ROOT", str(aster_root))
    monkeypatch.setattr("baybridge.backends.aster_exec.load_hip_library", lambda global_scope=True: object())
    return aster_root


def _matrix_f32() -> list[list[float]]:
    return [[float(4 * row + col) for col in range(4)] for row in range(4)]


def _matrix_i32() -> list[list[int]]:
    return [[4 * row + col for col in range(4)] for row in range(4)]


def _matrix_const_f32(value: float) -> list[list[float]]:
    return [[value for _ in range(4)] for _ in range(4)]


def _matrix_const_i32(value: int) -> list[list[int]]:
    return [[value for _ in range(4)] for _ in range(4)]


def _tail_matrix_f32() -> list[list[float]]:
    return [[float(5 * row + col) for col in range(5)] for row in range(2)]


def _tail_matrix_const_f32(value: float) -> list[list[float]]:
    return [[value for _ in range(5)] for _ in range(2)]


def _binary_nested(lhs: list[list[float | int]], rhs: list[list[float | int]], op: str) -> list[list[float | int]]:
    if op == "add":
        return [[left + right for left, right in zip(lhs_row, rhs_row, strict=True)] for lhs_row, rhs_row in zip(lhs, rhs, strict=True)]
    if op == "sub":
        return [[left - right for left, right in zip(lhs_row, rhs_row, strict=True)] for lhs_row, rhs_row in zip(lhs, rhs, strict=True)]
    if op == "mul":
        return [[left * right for left, right in zip(lhs_row, rhs_row, strict=True)] for lhs_row, rhs_row in zip(lhs, rhs, strict=True)]
    if isinstance(lhs[0][0], int) and isinstance(rhs[0][0], int):
        return [[left // right for left, right in zip(lhs_row, rhs_row, strict=True)] for lhs_row, rhs_row in zip(lhs, rhs, strict=True)]
    return [[left / right for left, right in zip(lhs_row, rhs_row, strict=True)] for lhs_row, rhs_row in zip(lhs, rhs, strict=True)]


F32_2D_BINARY_CASES = [
    ("add", aster_exec_add_2d_kernel, _matrix_const_f32(1.0)),
    ("sub", aster_exec_sub_2d_kernel, _matrix_const_f32(1.0)),
    ("mul", aster_exec_mul_2d_kernel, _matrix_const_f32(1.5)),
]

I32_2D_BINARY_CASES = [
    ("add", aster_exec_add_2d_i32_kernel, _matrix_const_i32(2)),
    ("sub", aster_exec_sub_2d_i32_kernel, _matrix_const_i32(2)),
    ("mul", aster_exec_mul_2d_i32_kernel, _matrix_const_i32(3)),
]

F32_TAIL_BINARY_CASES = [
    ("sub", aster_exec_sub_tail_kernel, _tail_matrix_const_f32(1.0)),
    ("mul", aster_exec_mul_tail_kernel, _tail_matrix_const_f32(1.5)),
]

I32_TAIL_BINARY_CASES = [
    ("sub", aster_exec_sub_i32_tail_kernel, [2 for _ in range(10)]),
    ("mul", aster_exec_mul_i32_tail_kernel, [3 for _ in range(10)]),
]

F32_BROADCAST_BINARY_CASES = [
    ("add", aster_exec_add_broadcast_kernel, [2.0]),
    ("sub", aster_exec_sub_broadcast_kernel, [2.0]),
    ("mul", aster_exec_mul_broadcast_kernel, [2.0]),
]

I32_BROADCAST_BINARY_CASES = [
    ("add", aster_exec_add_i32_broadcast_kernel, [2]),
    ("sub", aster_exec_sub_i32_broadcast_kernel, [2]),
    ("mul", aster_exec_mul_i32_broadcast_kernel, [2]),
]

F32_LHS_BROADCAST_BINARY_CASES = [
    ("sub", aster_exec_sub_lhs_broadcast_kernel, [100.0]),
]

I32_LHS_BROADCAST_BINARY_CASES = [
    ("sub", aster_exec_sub_i32_lhs_broadcast_kernel, [100]),
]

F32_2D_BROADCAST_CASES = [
    ("add", aster_exec_add_2d_broadcast_kernel, [[2.0]]),
    ("sub", aster_exec_sub_2d_lhs_broadcast_kernel, [[100.0]]),
]

I32_2D_BROADCAST_CASES = [
    ("add", aster_exec_add_2d_i32_broadcast_kernel, [[2]]),
    ("sub", aster_exec_sub_2d_i32_lhs_broadcast_kernel, [[100]]),
]


def test_aster_exec_lowers_copy_kernel(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    aster_root = _install_fake_aster(monkeypatch, tmp_path)

    artifact = bb.compile(aster_exec_copy_kernel, cache_dir=tmp_path / "cache", backend="aster_exec")

    assert artifact.lowered_module is not None
    assert artifact.lowered_module.dialect == "aster_exec_mlir"
    text = artifact.lowered_module.text
    assert "amdgcn.module @mod target = #amdgcn.target<" in text
    assert "#amdgcn.isa<" in text
    assert f"amdgcn.kernel @{aster_exec_copy_kernel.__name__}" in text
    assert "func.func private @copy_loop" in text
    assert str(aster_root) in os.environ["BAYBRIDGE_ASTER_ROOT"]
    assert artifact.debug_bundle_dir is not None
    assert (artifact.debug_bundle_dir / "kernel.mlir").exists()
    assert (artifact.debug_bundle_dir / "manifest.json").exists()
    assert (artifact.debug_bundle_dir / "repro.sh").exists()


def test_aster_exec_lowers_mfma_gemm_kernel(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _install_fake_aster(monkeypatch, tmp_path)

    artifact = bb.compile(aster_exec_mfma_gemm_kernel, cache_dir=tmp_path / "cache", backend="aster_exec")

    assert artifact.lowered_module is not None
    text = artifact.lowered_module.text
    assert "v_mfma_f32_16x16x16_f16" in text
    assert "global_load_dwordx2" in text
    assert "ds_write_b64" in text
    assert "ds_read_b64" in text
    assert "global_store_dwordx4" in text


def test_aster_exec_lowers_mfma_bf16_gemm_kernel(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _install_fake_aster(monkeypatch, tmp_path)

    artifact = bb.compile(aster_exec_mfma_bf16_gemm_kernel, cache_dir=tmp_path / "cache", backend="aster_exec")

    assert artifact.lowered_module is not None
    text = artifact.lowered_module.text
    assert "v_mfma_f32_16x16x16_bf16" in text
    assert "global_load_dwordx2" in text
    assert "ds_write_b64" in text
    assert "ds_read_b64" in text
    assert "global_store_dwordx4" in text


def test_aster_exec_lowers_mfma_fp8_gemm_kernel(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _install_fake_aster(monkeypatch, tmp_path)

    artifact = bb.compile(
        aster_exec_mfma_fp8_gemm_kernel,
        target=bb.AMDTarget(arch="gfx942"),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    assert artifact.lowered_module is not None
    text = artifact.lowered_module.text
    assert "v_mfma_f32_16x16x32_fp8_fp8" in text
    assert "global_load_dwordx2" in text
    assert "ds_write_b64" in text
    assert "ds_read_b64" in text
    assert "global_store_dwordx4" in text


def test_aster_exec_lowers_mfma_bf8_gemm_kernel(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _install_fake_aster(monkeypatch, tmp_path)

    artifact = bb.compile(
        aster_exec_mfma_bf8_gemm_kernel,
        target=bb.AMDTarget(arch="gfx942"),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    assert artifact.lowered_module is not None
    text = artifact.lowered_module.text
    assert "v_mfma_f32_16x16x32_bf8_bf8" in text
    assert "global_load_dwordx2" in text
    assert "ds_write_b64" in text
    assert "ds_read_b64" in text
    assert "global_store_dwordx4" in text


def test_aster_exec_lowers_mfma_fp8_bf8_gemm_kernel(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _install_fake_aster(monkeypatch, tmp_path)

    artifact = bb.compile(
        aster_exec_mfma_fp8_bf8_gemm_kernel,
        target=bb.AMDTarget(arch="gfx942"),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    assert artifact.lowered_module is not None
    text = artifact.lowered_module.text
    assert "v_mfma_f32_16x16x32_fp8_bf8" in text
    assert "global_load_dwordx2" in text
    assert "ds_write_b64" in text
    assert "ds_read_b64" in text
    assert "global_store_dwordx4" in text


def test_aster_exec_lowers_mfma_bf8_fp8_gemm_kernel(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _install_fake_aster(monkeypatch, tmp_path)

    artifact = bb.compile(
        aster_exec_mfma_bf8_fp8_gemm_kernel,
        target=bb.AMDTarget(arch="gfx942"),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    assert artifact.lowered_module is not None
    text = artifact.lowered_module.text
    assert "v_mfma_f32_16x16x32_bf8_fp8" in text
    assert "global_load_dwordx2" in text
    assert "ds_write_b64" in text
    assert "ds_read_b64" in text
    assert "global_store_dwordx4" in text


def test_aster_exec_lowers_fragment_mfma_kernel(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _install_fake_aster(monkeypatch, tmp_path)

    artifact = bb.compile(aster_exec_fragment_mfma_kernel, cache_dir=tmp_path / "cache", backend="aster_exec")

    assert artifact.lowered_module is not None
    text = artifact.lowered_module.text
    assert "v_mfma_f32_16x16x16_f16" in text
    assert "global_load_dwordx2" in text
    assert "ds_write_b64" in text
    assert "global_store_dwordx4" in text


def test_aster_exec_lowers_fragment_mfma_bf16_kernel(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _install_fake_aster(monkeypatch, tmp_path)

    artifact = bb.compile(aster_exec_fragment_mfma_bf16_kernel, cache_dir=tmp_path / "cache", backend="aster_exec")

    assert artifact.lowered_module is not None
    text = artifact.lowered_module.text
    assert "v_mfma_f32_16x16x16_bf16" in text
    assert "global_load_dwordx2" in text
    assert "ds_write_b64" in text
    assert "global_store_dwordx4" in text


def test_aster_exec_launches_copy_kernel_with_fake_aster(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([float(index) for index in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_copy_kernel,
        src,
        dst,
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, dst)

    assert dst.tolist() == src.tolist()
    assert artifact.lowered_path is not None
    assert artifact.lowered_path.with_suffix(".hsaco").exists()


def test_aster_exec_launches_mfma_gemm_kernel_with_fake_aster(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_gemm_kernel,
        a,
        b,
        c,
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(a, b, c)

    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]
    assert artifact.lowered_path is not None
    assert artifact.lowered_path.with_suffix(".hsaco").exists()


def test_aster_exec_launches_mfma_bf16_gemm_kernel_with_fake_aster(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_bf16_gemm_kernel,
        a,
        b,
        c,
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(a, b, c)

    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]
    assert artifact.lowered_path is not None
    assert artifact.lowered_path.with_suffix(".hsaco").exists()


def test_aster_exec_launches_mfma_fp8_gemm_kernel_with_fake_aster(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    a = bb.tensor([[0x40 for _ in range(32)] for _ in range(16)], dtype="fp8")
    b = bb.tensor([[0x40 for _ in range(16)] for _ in range(32)], dtype="fp8")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_fp8_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch="gfx942"),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(a, b, c)

    assert c.tolist() == [[32.0 for _ in range(16)] for _ in range(16)]
    assert artifact.lowered_path is not None
    assert artifact.lowered_path.with_suffix(".hsaco").exists()


def test_aster_exec_launches_mfma_bf8_gemm_kernel_with_fake_aster(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    a = bb.tensor([[0x40 for _ in range(32)] for _ in range(16)], dtype="bf8")
    b = bb.tensor([[0x40 for _ in range(16)] for _ in range(32)], dtype="bf8")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_bf8_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch="gfx942"),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(a, b, c)

    assert c.tolist() == [[32.0 for _ in range(16)] for _ in range(16)]
    assert artifact.lowered_path is not None
    assert artifact.lowered_path.with_suffix(".hsaco").exists()


def test_aster_exec_launches_mfma_fp8_bf8_gemm_kernel_with_fake_aster(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    a = bb.tensor([[0x40 for _ in range(32)] for _ in range(16)], dtype="fp8")
    b = bb.tensor([[0x40 for _ in range(16)] for _ in range(32)], dtype="bf8")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_fp8_bf8_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch="gfx942"),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(a, b, c)

    assert c.tolist() == [[32.0 for _ in range(16)] for _ in range(16)]
    assert artifact.lowered_path is not None
    assert artifact.lowered_path.with_suffix(".hsaco").exists()


def test_aster_exec_launches_mfma_bf8_fp8_gemm_kernel_with_fake_aster(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    a = bb.tensor([[0x40 for _ in range(32)] for _ in range(16)], dtype="bf8")
    b = bb.tensor([[0x40 for _ in range(16)] for _ in range(32)], dtype="fp8")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_bf8_fp8_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch="gfx942"),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(a, b, c)

    assert c.tolist() == [[32.0 for _ in range(16)] for _ in range(16)]
    assert artifact.lowered_path is not None
    assert artifact.lowered_path.with_suffix(".hsaco").exists()


def test_aster_exec_launches_fragment_mfma_kernel_with_fake_aster(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_fragment_mfma_kernel,
        a,
        b,
        c,
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(a, b, c)

    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_aster_exec_launches_fragment_mfma_bf16_kernel_with_fake_aster(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_fragment_mfma_bf16_kernel,
        a,
        b,
        c,
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(a, b, c)

    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_matching_copy_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([float(index) for index in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_copy_kernel,
        src,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_compile_auto_prefers_aster_exec_for_matching_mfma_gemm_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_gemm_kernel,
        a,
        b,
        c,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_matching_mfma_bf16_gemm_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_bf16_gemm_kernel,
        a,
        b,
        c,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_matching_mfma_fp8_gemm_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    a = bb.tensor([[0x40 for _ in range(32)] for _ in range(16)], dtype="fp8")
    b = bb.tensor([[0x40 for _ in range(16)] for _ in range(32)], dtype="fp8")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_fp8_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch="gfx942"),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[32.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_matching_mfma_bf8_gemm_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    a = bb.tensor([[0x40 for _ in range(32)] for _ in range(16)], dtype="bf8")
    b = bb.tensor([[0x40 for _ in range(16)] for _ in range(32)], dtype="bf8")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_bf8_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch="gfx942"),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[32.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_matching_mfma_fp8_bf8_gemm_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    a = bb.tensor([[0x40 for _ in range(32)] for _ in range(16)], dtype="fp8")
    b = bb.tensor([[0x40 for _ in range(16)] for _ in range(32)], dtype="bf8")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_fp8_bf8_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch="gfx942"),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[32.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_matching_mfma_bf8_fp8_gemm_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    a = bb.tensor([[0x40 for _ in range(32)] for _ in range(16)], dtype="bf8")
    b = bb.tensor([[0x40 for _ in range(16)] for _ in range(32)], dtype="fp8")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_bf8_fp8_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch="gfx942"),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[32.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_matching_fragment_mfma_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_fragment_mfma_kernel,
        a,
        b,
        c,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_matching_fragment_mfma_bf16_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_fragment_mfma_bf16_kernel,
        a,
        b,
        c,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_matching_tcgen05_fragment_mfma_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_tcgen05_fragment_mfma_kernel,
        a,
        b,
        c,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_matching_warp_fragment_mfma_bf16_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_warp_fragment_mfma_bf16_kernel,
        a,
        b,
        c,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_matching_copy_tail_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([float(index) for index in range(10)], dtype="f32")
    dst = bb.zeros((10,), dtype="f32")

    artifact = bb.compile(
        aster_exec_copy_tail_kernel,
        src,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_compile_does_not_auto_prefer_aster_exec_for_unsupported_ir(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([float(index) for index in range(16)], dtype="f32")
    other = bb.tensor([1.0 for _ in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_unsupported_pipeline_kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name != "aster_exec"


def test_compile_does_not_auto_prefer_aster_exec_for_unsupported_pipeline(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([float(index) for index in range(16)], dtype="f32")
    other = bb.tensor([1.0 for _ in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_unsupported_pipeline_kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name != "aster_exec"


def test_aster_exec_rejects_unsupported_ir(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([float(index) for index in range(16)], dtype="f32")
    other = bb.tensor([1.0 for _ in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    with pytest.raises(bb.BackendNotImplementedError, match="single supported tensor binary pipeline"):
        bb.compile(
            aster_exec_unsupported_pipeline_kernel,
            src,
            other,
            dst,
            cache_dir=tmp_path / "cache",
            backend="aster_exec",
        )


def test_compile_auto_prefers_aster_exec_for_matching_add_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([float(index) for index in range(16)], dtype="f32")
    other = bb.tensor([1.0 for _ in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_add_kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [left + right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


def test_compile_auto_prefers_aster_exec_for_matching_sub_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([float(index) for index in range(16)], dtype="f32")
    other = bb.tensor([1.0 for _ in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_sub_kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [left - right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


def test_compile_auto_prefers_aster_exec_for_matching_mul_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([float(index) for index in range(16)], dtype="f32")
    other = bb.tensor([1.5 for _ in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_mul_kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [left * right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


def test_compile_does_not_auto_prefer_aster_exec_for_div_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([float(index + 2) for index in range(16)], dtype="f32")
    other = bb.tensor([2.0 for _ in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_div_kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name != "aster_exec"


def test_compile_auto_prefers_aster_exec_for_matching_i32_copy_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor(list(range(16)), dtype="i32")
    dst = bb.zeros((16,), dtype="i32")

    artifact = bb.compile(
        aster_exec_copy_i32_kernel,
        src,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_compile_auto_prefers_aster_exec_for_matching_i32_add_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor(list(range(16)), dtype="i32")
    other = bb.tensor([2 for _ in range(16)], dtype="i32")
    dst = bb.zeros((16,), dtype="i32")

    artifact = bb.compile(
        aster_exec_add_i32_kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [left + right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


def test_compile_auto_prefers_aster_exec_for_matching_add_tail_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([[float(5 * row + col) for col in range(5)] for row in range(2)], dtype="f32")
    other = bb.tensor([[1.0 for _ in range(5)] for _ in range(2)], dtype="f32")
    dst = bb.zeros((2, 5), dtype="f32")

    artifact = bb.compile(
        aster_exec_add_tail_kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == _binary_nested(src.tolist(), other.tolist(), "add")


def test_compile_auto_prefers_aster_exec_for_matching_i32_add_tail_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor(list(range(10)), dtype="i32")
    other = bb.tensor([2 for _ in range(10)], dtype="i32")
    dst = bb.zeros((10,), dtype="i32")

    artifact = bb.compile(
        aster_exec_add_i32_tail_kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [left + right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


@pytest.mark.parametrize("op_name,kernel,other_values", F32_TAIL_BINARY_CASES)
def test_compile_auto_prefers_aster_exec_for_matching_other_tail_f32_binary_kernels(
    op_name: str,
    kernel,
    other_values: list[list[float]],
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor(_tail_matrix_f32(), dtype="f32")
    other = bb.tensor(other_values, dtype="f32")
    dst = bb.zeros((2, 5), dtype="f32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == _binary_nested(src.tolist(), other.tolist(), op_name)


@pytest.mark.parametrize("op_name,kernel,other_values", I32_TAIL_BINARY_CASES)
def test_compile_auto_prefers_aster_exec_for_matching_other_tail_i32_binary_kernels(
    op_name: str,
    kernel,
    other_values: list[int],
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor(list(range(10)), dtype="i32")
    other = bb.tensor(other_values, dtype="i32")
    dst = bb.zeros((10,), dtype="i32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    if op_name == "sub":
        expected = [left - right for left, right in zip(src.tolist(), other.tolist(), strict=True)]
    elif op_name == "mul":
        expected = [left * right for left, right in zip(src.tolist(), other.tolist(), strict=True)]
    else:
        expected = [left // right for left, right in zip(src.tolist(), other.tolist(), strict=True)]
    assert dst.tolist() == expected


@pytest.mark.parametrize("op_name,kernel,other_values", F32_BROADCAST_BINARY_CASES)
def test_compile_auto_prefers_aster_exec_for_matching_f32_broadcast_binary_kernels(
    op_name: str,
    kernel,
    other_values: list[float],
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([float(index) for index in range(10)], dtype="f32")
    other = bb.tensor(other_values, dtype="f32")
    dst = bb.zeros((10,), dtype="f32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    scalar = other.tolist()[0]
    if op_name == "add":
        expected = [left + scalar for left in src.tolist()]
    elif op_name == "sub":
        expected = [left - scalar for left in src.tolist()]
    elif op_name == "mul":
        expected = [left * scalar for left in src.tolist()]
    else:
        expected = [left / scalar for left in src.tolist()]
    assert dst.tolist() == expected


@pytest.mark.parametrize("op_name,kernel,other_values", I32_BROADCAST_BINARY_CASES)
def test_compile_auto_prefers_aster_exec_for_matching_i32_broadcast_binary_kernels(
    op_name: str,
    kernel,
    other_values: list[int],
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor(list(range(10)), dtype="i32")
    other = bb.tensor(other_values, dtype="i32")
    dst = bb.zeros((10,), dtype="i32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    scalar = other.tolist()[0]
    if op_name == "add":
        expected = [left + scalar for left in src.tolist()]
    elif op_name == "sub":
        expected = [left - scalar for left in src.tolist()]
    elif op_name == "mul":
        expected = [left * scalar for left in src.tolist()]
    else:
        expected = [left // scalar for left in src.tolist()]
    assert dst.tolist() == expected


def test_compile_auto_prefers_aster_exec_for_matching_aligned_f32_broadcast_binary_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([float(index) for index in range(16)], dtype="f32")
    other = bb.tensor([2.0], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_add_broadcast_aligned_kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [left + 2.0 for left in src.tolist()]


def test_compile_auto_prefers_aster_exec_for_matching_aligned_i32_broadcast_binary_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor(list(range(16)), dtype="i32")
    other = bb.tensor([2], dtype="i32")
    dst = bb.zeros((16,), dtype="i32")

    artifact = bb.compile(
        aster_exec_add_i32_broadcast_aligned_kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [left + 2 for left in src.tolist()]


@pytest.mark.parametrize("op_name,kernel,src_values", F32_LHS_BROADCAST_BINARY_CASES)
def test_compile_auto_prefers_aster_exec_for_matching_f32_lhs_broadcast_binary_kernels(
    op_name: str,
    kernel,
    src_values: list[float],
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor(src_values, dtype="f32")
    other = bb.tensor([float(index + 1) for index in range(10)], dtype="f32")
    dst = bb.zeros((10,), dtype="f32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    scalar = src.tolist()[0]
    if op_name == "sub":
        expected = [scalar - right for right in other.tolist()]
    else:
        expected = [scalar / right for right in other.tolist()]
    assert dst.tolist() == expected


@pytest.mark.parametrize("op_name,kernel,src_values", I32_LHS_BROADCAST_BINARY_CASES)
def test_compile_auto_prefers_aster_exec_for_matching_i32_lhs_broadcast_binary_kernels(
    op_name: str,
    kernel,
    src_values: list[int],
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor(src_values, dtype="i32")
    other = bb.tensor([index + 1 for index in range(10)], dtype="i32")
    dst = bb.zeros((10,), dtype="i32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    scalar = src.tolist()[0]
    if op_name == "sub":
        expected = [scalar - right for right in other.tolist()]
    else:
        expected = [scalar // right for right in other.tolist()]
    assert dst.tolist() == expected


@pytest.mark.parametrize("op_name,kernel,scalar_values", F32_2D_BROADCAST_CASES)
def test_compile_auto_prefers_aster_exec_for_matching_2d_f32_broadcast_kernels(
    op_name: str,
    kernel,
    scalar_values: list[list[float]],
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    dense = bb.tensor(_tail_matrix_f32(), dtype="f32")
    scalar = bb.tensor(scalar_values, dtype="f32")
    dst = bb.zeros((2, 5), dtype="f32")

    if op_name == "add":
        artifact = bb.compile(kernel, dense, scalar, dst, cache_dir=tmp_path / "cache")
        artifact(dense, scalar, dst)
        expected = _binary_nested(dense.tolist(), [[scalar.tolist()[0][0] for _ in range(5)] for _ in range(2)], "add")
    else:
        artifact = bb.compile(kernel, scalar, dense, dst, cache_dir=tmp_path / "cache")
        artifact(scalar, dense, dst)
        scalar_value = scalar.tolist()[0][0]
        expected = [[scalar_value - right for right in row] for row in dense.tolist()]

    assert artifact.backend_name == "aster_exec"
    assert dst.tolist() == expected


@pytest.mark.parametrize("op_name,kernel,scalar_values", I32_2D_BROADCAST_CASES)
def test_compile_auto_prefers_aster_exec_for_matching_2d_i32_broadcast_kernels(
    op_name: str,
    kernel,
    scalar_values: list[list[int]],
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    dense = bb.tensor([[5 * row + col for col in range(5)] for row in range(2)], dtype="i32")
    scalar = bb.tensor(scalar_values, dtype="i32")
    dst = bb.zeros((2, 5), dtype="i32")

    if op_name == "add":
        artifact = bb.compile(kernel, dense, scalar, dst, cache_dir=tmp_path / "cache")
        artifact(dense, scalar, dst)
        expected = _binary_nested(dense.tolist(), [[scalar.tolist()[0][0] for _ in range(5)] for _ in range(2)], "add")
    else:
        artifact = bb.compile(kernel, scalar, dense, dst, cache_dir=tmp_path / "cache")
        artifact(scalar, dense, dst)
        scalar_value = scalar.tolist()[0][0]
        expected = [[scalar_value - right for right in row] for row in dense.tolist()]

    assert artifact.backend_name == "aster_exec"
    assert dst.tolist() == expected


def test_compile_auto_prefers_aster_exec_for_matching_i32_sub_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor(list(range(16)), dtype="i32")
    other = bb.tensor([2 for _ in range(16)], dtype="i32")
    dst = bb.zeros((16,), dtype="i32")

    artifact = bb.compile(
        aster_exec_sub_i32_kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [left - right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


def test_compile_auto_prefers_aster_exec_for_matching_i32_mul_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor(list(range(16)), dtype="i32")
    other = bb.tensor([3 for _ in range(16)], dtype="i32")
    dst = bb.zeros((16,), dtype="i32")

    artifact = bb.compile(
        aster_exec_mul_i32_kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [left * right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


def test_compile_does_not_auto_prefer_aster_exec_for_matching_i32_div_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([2 * (index + 1) for index in range(16)], dtype="i32")
    other = bb.tensor([2 for _ in range(16)], dtype="i32")
    dst = bb.zeros((16,), dtype="i32")

    artifact = bb.compile(
        aster_exec_div_i32_kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name != "aster_exec"


def test_compile_auto_prefers_aster_exec_for_matching_f16_copy_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([index / 8.0 for index in range(16)], dtype="f16")
    dst = bb.zeros((16,), dtype="f16")

    artifact = bb.compile(
        aster_exec_copy_f16_kernel,
        src,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_compile_auto_prefers_aster_exec_for_matching_2d_copy_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([[float(4 * row + col) for col in range(4)] for row in range(4)], dtype="f32")
    dst = bb.zeros((4, 4), dtype="f32")

    artifact = bb.compile(
        aster_exec_copy_2d_kernel,
        src,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_compile_auto_prefers_aster_exec_for_matching_2d_add_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([[float(4 * row + col) for col in range(4)] for row in range(4)], dtype="f32")
    other = bb.tensor([[1.0 for _ in range(4)] for _ in range(4)], dtype="f32")
    dst = bb.zeros((4, 4), dtype="f32")

    artifact = bb.compile(
        aster_exec_add_2d_kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [[left + right for left, right in zip(src_row, other_row, strict=True)] for src_row, other_row in zip(src.tolist(), other.tolist(), strict=True)]


def test_compile_auto_prefers_aster_exec_for_matching_f16_tail_copy_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([index / 8.0 for index in range(10)], dtype="f16")
    dst = bb.zeros((10,), dtype="f16")

    artifact = bb.compile(
        aster_exec_copy_f16_tail_kernel,
        src,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_compile_auto_prefers_aster_exec_for_matching_2d_f16_copy_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([[float(4 * row + col) / 8.0 for col in range(4)] for row in range(4)], dtype="f16")
    dst = bb.zeros((4, 4), dtype="f16")

    artifact = bb.compile(
        aster_exec_copy_2d_f16_kernel,
        src,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_compile_auto_prefers_aster_exec_for_matching_2d_f16_tail_copy_kernel(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor([[float(5 * row + col) / 8.0 for col in range(5)] for row in range(2)], dtype="f16")
    dst = bb.zeros((2, 5), dtype="f16")

    artifact = bb.compile(
        aster_exec_copy_2d_f16_tail_kernel,
        src,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, dst)
    assert dst.tolist() == src.tolist()


@pytest.mark.parametrize("op_name,kernel,other_matrix", F32_2D_BINARY_CASES[1:])
def test_compile_auto_prefers_aster_exec_for_matching_other_2d_f32_binary_kernels(
    op_name: str,
    kernel,
    other_matrix: list[list[float]],
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor(_matrix_f32(), dtype="f32")
    other = bb.tensor(other_matrix, dtype="f32")
    dst = bb.zeros((4, 4), dtype="f32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == _binary_nested(src.tolist(), other.tolist(), op_name)


@pytest.mark.parametrize("op_name,kernel,other_matrix", I32_2D_BINARY_CASES)
def test_compile_auto_prefers_aster_exec_for_matching_2d_i32_binary_kernels(
    op_name: str,
    kernel,
    other_matrix: list[list[int]],
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _install_fake_aster(monkeypatch, tmp_path)
    src = bb.tensor(_matrix_i32(), dtype="i32")
    other = bb.tensor(other_matrix, dtype="i32")
    dst = bb.zeros((4, 4), dtype="i32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == _binary_nested(src.tolist(), other.tolist(), op_name)


def test_aster_exec_runs_copy_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([float(index) for index in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_copy_kernel,
        src,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, dst)

    assert dst.tolist() == src.tolist()


def test_aster_exec_runs_mfma_gemm_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(a, b, c)
    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_aster_exec_runs_mfma_bf16_gemm_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_bf16_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(a, b, c)
    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_aster_exec_runs_mfma_fp8_gemm_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    if target_arch != "gfx942":
        pytest.skip("aster_exec fp8 MFMA currently supports gfx942 only")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[0x40 for _ in range(32)] for _ in range(16)], dtype="fp8")
    b = bb.tensor([[0x40 for _ in range(16)] for _ in range(32)], dtype="fp8")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_fp8_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(a, b, c)
    assert c.tolist() == [[32.0 for _ in range(16)] for _ in range(16)]


def test_aster_exec_runs_mfma_bf8_gemm_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    if target_arch != "gfx942":
        pytest.skip("aster_exec bf8 MFMA currently supports gfx942 only")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[0x40 for _ in range(32)] for _ in range(16)], dtype="bf8")
    b = bb.tensor([[0x40 for _ in range(16)] for _ in range(32)], dtype="bf8")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_bf8_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(a, b, c)
    assert c.tolist() == [[32.0 for _ in range(16)] for _ in range(16)]


def test_aster_exec_runs_mfma_fp8_bf8_gemm_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    if target_arch != "gfx942":
        pytest.skip("aster_exec fp8/bf8 MFMA currently supports gfx942 only")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[0x40 for _ in range(32)] for _ in range(16)], dtype="fp8")
    b = bb.tensor([[0x40 for _ in range(16)] for _ in range(32)], dtype="bf8")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_fp8_bf8_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(a, b, c)
    assert c.tolist() == [[32.0 for _ in range(16)] for _ in range(16)]


def test_aster_exec_runs_mfma_bf8_fp8_gemm_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    if target_arch != "gfx942":
        pytest.skip("aster_exec bf8/fp8 MFMA currently supports gfx942 only")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[0x40 for _ in range(32)] for _ in range(16)], dtype="bf8")
    b = bb.tensor([[0x40 for _ in range(16)] for _ in range(32)], dtype="fp8")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_bf8_fp8_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(a, b, c)
    assert c.tolist() == [[32.0 for _ in range(16)] for _ in range(16)]


def test_aster_exec_runs_fragment_mfma_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_fragment_mfma_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(a, b, c)
    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_aster_exec_runs_fragment_mfma_bf16_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_fragment_mfma_bf16_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(a, b, c)
    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_aster_exec_runs_tcgen05_fragment_mfma_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_tcgen05_fragment_mfma_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(a, b, c)
    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_aster_exec_runs_warp_fragment_mfma_bf16_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_warp_fragment_mfma_bf16_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(a, b, c)
    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_aster_exec_runs_add_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([float(index) for index in range(16)], dtype="f32")
    other = bb.tensor([1.0 for _ in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_add_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, other, dst)
    assert dst.tolist() == [left + right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


def test_aster_exec_runs_2d_copy_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([[float(4 * row + col) for col in range(4)] for row in range(4)], dtype="f32")
    dst = bb.zeros((4, 4), dtype="f32")

    artifact = bb.compile(
        aster_exec_copy_2d_kernel,
        src,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_aster_exec_runs_copy_tail_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([float(index) for index in range(10)], dtype="f32")
    dst = bb.zeros((10,), dtype="f32")

    artifact = bb.compile(
        aster_exec_copy_tail_kernel,
        src,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_aster_exec_runs_2d_add_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([[float(4 * row + col) for col in range(4)] for row in range(4)], dtype="f32")
    other = bb.tensor([[1.0 for _ in range(4)] for _ in range(4)], dtype="f32")
    dst = bb.zeros((4, 4), dtype="f32")

    artifact = bb.compile(
        aster_exec_add_2d_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, other, dst)
    assert dst.tolist() == [[left + right for left, right in zip(src_row, other_row, strict=True)] for src_row, other_row in zip(src.tolist(), other.tolist(), strict=True)]


def test_aster_exec_runs_2d_f16_copy_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([[float(4 * row + col) / 8.0 for col in range(4)] for row in range(4)], dtype="f16")
    dst = bb.zeros((4, 4), dtype="f16")

    artifact = bb.compile(
        aster_exec_copy_2d_f16_kernel,
        src,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_aster_exec_runs_2d_f16_tail_copy_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([[float(5 * row + col) / 8.0 for col in range(5)] for row in range(2)], dtype="f16")
    dst = bb.zeros((2, 5), dtype="f16")

    artifact = bb.compile(
        aster_exec_copy_2d_f16_tail_kernel,
        src,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, dst)
    assert dst.tolist() == src.tolist()


@pytest.mark.parametrize("op_name,kernel,other_matrix", F32_2D_BINARY_CASES[1:])
def test_aster_exec_runs_other_2d_f32_binary_on_amd_hardware(
    op_name: str,
    kernel,
    other_matrix: list[list[float]],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(_matrix_f32(), dtype="f32")
    other = bb.tensor(other_matrix, dtype="f32")
    dst = bb.zeros((4, 4), dtype="f32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, other, dst)
    assert dst.tolist() == _binary_nested(src.tolist(), other.tolist(), op_name)


@pytest.mark.parametrize("op_name,kernel,other_matrix", I32_2D_BINARY_CASES)
def test_aster_exec_runs_2d_i32_binary_on_amd_hardware(
    op_name: str,
    kernel,
    other_matrix: list[list[int]],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(_matrix_i32(), dtype="i32")
    other = bb.tensor(other_matrix, dtype="i32")
    dst = bb.zeros((4, 4), dtype="i32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, other, dst)
    assert dst.tolist() == _binary_nested(src.tolist(), other.tolist(), op_name)


def test_aster_exec_runs_sub_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([float(index) for index in range(16)], dtype="f32")
    other = bb.tensor([1.0 for _ in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_sub_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, other, dst)
    assert dst.tolist() == [left - right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


def test_aster_exec_runs_mul_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([float(index) for index in range(16)], dtype="f32")
    other = bb.tensor([1.5 for _ in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_mul_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, other, dst)
    assert dst.tolist() == [left * right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


def test_aster_exec_rejects_div_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([float(index + 2) for index in range(16)], dtype="f32")
    other = bb.tensor([2.0 for _ in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    with pytest.raises(bb.BackendNotImplementedError):
        bb.compile(
            aster_exec_div_kernel,
            src,
            other,
            dst,
            target=bb.AMDTarget(arch=target_arch),
            cache_dir=tmp_path / "cache",
            backend="aster_exec",
        )


def test_aster_exec_runs_i32_copy_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(list(range(16)), dtype="i32")
    dst = bb.zeros((16,), dtype="i32")

    artifact = bb.compile(
        aster_exec_copy_i32_kernel,
        src,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_aster_exec_runs_i32_add_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(list(range(16)), dtype="i32")
    other = bb.tensor([2 for _ in range(16)], dtype="i32")
    dst = bb.zeros((16,), dtype="i32")

    artifact = bb.compile(
        aster_exec_add_i32_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, other, dst)
    assert dst.tolist() == [left + right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


def test_aster_exec_runs_add_tail_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([[float(5 * row + col) for col in range(5)] for row in range(2)], dtype="f32")
    other = bb.tensor([[1.0 for _ in range(5)] for _ in range(2)], dtype="f32")
    dst = bb.zeros((2, 5), dtype="f32")

    artifact = bb.compile(
        aster_exec_add_tail_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, other, dst)
    assert dst.tolist() == _binary_nested(src.tolist(), other.tolist(), "add")


def test_aster_exec_runs_i32_add_tail_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(list(range(10)), dtype="i32")
    other = bb.tensor([2 for _ in range(10)], dtype="i32")
    dst = bb.zeros((10,), dtype="i32")

    artifact = bb.compile(
        aster_exec_add_i32_tail_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, other, dst)
    assert dst.tolist() == [left + right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


@pytest.mark.parametrize("op_name,kernel,other_values", F32_TAIL_BINARY_CASES)
def test_aster_exec_runs_other_tail_f32_binary_on_amd_hardware(
    op_name: str,
    kernel,
    other_values: list[list[float]],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(_tail_matrix_f32(), dtype="f32")
    other = bb.tensor(other_values, dtype="f32")
    dst = bb.zeros((2, 5), dtype="f32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, other, dst)
    assert dst.tolist() == _binary_nested(src.tolist(), other.tolist(), op_name)


@pytest.mark.parametrize("op_name,kernel,other_values", I32_TAIL_BINARY_CASES)
def test_aster_exec_runs_other_tail_i32_binary_on_amd_hardware(
    op_name: str,
    kernel,
    other_values: list[int],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(list(range(10)), dtype="i32")
    other = bb.tensor(other_values, dtype="i32")
    dst = bb.zeros((10,), dtype="i32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, other, dst)
    if op_name == "sub":
        expected = [left - right for left, right in zip(src.tolist(), other.tolist(), strict=True)]
    elif op_name == "mul":
        expected = [left * right for left, right in zip(src.tolist(), other.tolist(), strict=True)]
    else:
        expected = [left // right for left, right in zip(src.tolist(), other.tolist(), strict=True)]
    assert dst.tolist() == expected


@pytest.mark.parametrize("op_name,kernel,other_values", F32_BROADCAST_BINARY_CASES)
def test_aster_exec_runs_f32_broadcast_binary_on_amd_hardware(
    op_name: str,
    kernel,
    other_values: list[float],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([float(index) for index in range(10)], dtype="f32")
    other = bb.tensor(other_values, dtype="f32")
    dst = bb.zeros((10,), dtype="f32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, other, dst)
    scalar = other.tolist()[0]
    if op_name == "add":
        expected = [left + scalar for left in src.tolist()]
    elif op_name == "sub":
        expected = [left - scalar for left in src.tolist()]
    elif op_name == "mul":
        expected = [left * scalar for left in src.tolist()]
    else:
        expected = [left / scalar for left in src.tolist()]
    assert dst.tolist() == expected


def test_aster_exec_runs_aligned_f32_broadcast_binary_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([float(index) for index in range(16)], dtype="f32")
    other = bb.tensor([2.0], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_add_broadcast_aligned_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [left + 2.0 for left in src.tolist()]


def test_aster_exec_runs_aligned_i32_broadcast_binary_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(list(range(16)), dtype="i32")
    other = bb.tensor([2], dtype="i32")
    dst = bb.zeros((16,), dtype="i32")

    artifact = bb.compile(
        aster_exec_add_i32_broadcast_aligned_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [left + 2 for left in src.tolist()]


@pytest.mark.parametrize("op_name,kernel,other_values", I32_BROADCAST_BINARY_CASES)
def test_aster_exec_runs_i32_broadcast_binary_on_amd_hardware(
    op_name: str,
    kernel,
    other_values: list[int],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(list(range(10)), dtype="i32")
    other = bb.tensor(other_values, dtype="i32")
    dst = bb.zeros((10,), dtype="i32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, other, dst)
    scalar = other.tolist()[0]
    if op_name == "add":
        expected = [left + scalar for left in src.tolist()]
    elif op_name == "sub":
        expected = [left - scalar for left in src.tolist()]
    elif op_name == "mul":
        expected = [left * scalar for left in src.tolist()]
    else:
        expected = [left // scalar for left in src.tolist()]
    assert dst.tolist() == expected


@pytest.mark.parametrize("op_name,kernel,src_values", F32_LHS_BROADCAST_BINARY_CASES)
def test_aster_exec_runs_f32_lhs_broadcast_binary_on_amd_hardware(
    op_name: str,
    kernel,
    src_values: list[float],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(src_values, dtype="f32")
    other = bb.tensor([float(index + 1) for index in range(10)], dtype="f32")
    dst = bb.zeros((10,), dtype="f32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, other, dst)
    scalar = src.tolist()[0]
    if op_name == "sub":
        expected = [scalar - right for right in other.tolist()]
    else:
        expected = [scalar / right for right in other.tolist()]
    assert dst.tolist() == expected


@pytest.mark.parametrize("op_name,kernel,src_values", I32_LHS_BROADCAST_BINARY_CASES)
def test_aster_exec_runs_i32_lhs_broadcast_binary_on_amd_hardware(
    op_name: str,
    kernel,
    src_values: list[int],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(src_values, dtype="i32")
    other = bb.tensor([index + 1 for index in range(10)], dtype="i32")
    dst = bb.zeros((10,), dtype="i32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, other, dst)
    scalar = src.tolist()[0]
    if op_name == "sub":
        expected = [scalar - right for right in other.tolist()]
    else:
        expected = [scalar // right for right in other.tolist()]
    assert dst.tolist() == expected


@pytest.mark.parametrize("op_name,kernel,scalar_values", F32_2D_BROADCAST_CASES)
def test_aster_exec_runs_2d_f32_broadcast_on_amd_hardware(
    op_name: str,
    kernel,
    scalar_values: list[list[float]],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    dense = bb.tensor(_tail_matrix_f32(), dtype="f32")
    scalar = bb.tensor(scalar_values, dtype="f32")
    dst = bb.zeros((2, 5), dtype="f32")

    if op_name == "add":
        artifact = bb.compile(kernel, dense, scalar, dst, target=bb.AMDTarget(arch=target_arch), cache_dir=tmp_path / "cache", backend="aster_exec")
        artifact(dense, scalar, dst)
        expected = _binary_nested(dense.tolist(), [[scalar.tolist()[0][0] for _ in range(5)] for _ in range(2)], "add")
    else:
        artifact = bb.compile(kernel, scalar, dense, dst, target=bb.AMDTarget(arch=target_arch), cache_dir=tmp_path / "cache", backend="aster_exec")
        artifact(scalar, dense, dst)
        scalar_value = scalar.tolist()[0][0]
        expected = [[scalar_value - right for right in row] for row in dense.tolist()]

    assert dst.tolist() == expected


@pytest.mark.parametrize("op_name,kernel,scalar_values", I32_2D_BROADCAST_CASES)
def test_aster_exec_runs_2d_i32_broadcast_on_amd_hardware(
    op_name: str,
    kernel,
    scalar_values: list[list[int]],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    dense = bb.tensor([[5 * row + col for col in range(5)] for row in range(2)], dtype="i32")
    scalar = bb.tensor(scalar_values, dtype="i32")
    dst = bb.zeros((2, 5), dtype="i32")

    if op_name == "add":
        artifact = bb.compile(kernel, dense, scalar, dst, target=bb.AMDTarget(arch=target_arch), cache_dir=tmp_path / "cache", backend="aster_exec")
        artifact(dense, scalar, dst)
        expected = _binary_nested(dense.tolist(), [[scalar.tolist()[0][0] for _ in range(5)] for _ in range(2)], "add")
    else:
        artifact = bb.compile(kernel, scalar, dense, dst, target=bb.AMDTarget(arch=target_arch), cache_dir=tmp_path / "cache", backend="aster_exec")
        artifact(scalar, dense, dst)
        scalar_value = scalar.tolist()[0][0]
        expected = [[scalar_value - right for right in row] for row in dense.tolist()]

    assert dst.tolist() == expected


def test_aster_exec_runs_i32_sub_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(list(range(16)), dtype="i32")
    other = bb.tensor([2 for _ in range(16)], dtype="i32")
    dst = bb.zeros((16,), dtype="i32")

    artifact = bb.compile(
        aster_exec_sub_i32_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, other, dst)
    assert dst.tolist() == [left - right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


def test_aster_exec_runs_i32_mul_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(list(range(16)), dtype="i32")
    other = bb.tensor([3 for _ in range(16)], dtype="i32")
    dst = bb.zeros((16,), dtype="i32")

    artifact = bb.compile(
        aster_exec_mul_i32_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, other, dst)
    assert dst.tolist() == [left * right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


def test_aster_exec_rejects_i32_div_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([2 * (index + 1) for index in range(16)], dtype="i32")
    other = bb.tensor([2 for _ in range(16)], dtype="i32")
    dst = bb.zeros((16,), dtype="i32")

    with pytest.raises(bb.BackendNotImplementedError):
        bb.compile(
            aster_exec_div_i32_kernel,
            src,
            other,
            dst,
            target=bb.AMDTarget(arch=target_arch),
            cache_dir=tmp_path / "cache",
            backend="aster_exec",
        )


def test_aster_exec_runs_f16_copy_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([index / 8.0 for index in range(16)], dtype="f16")
    dst = bb.zeros((16,), dtype="f16")

    artifact = bb.compile(
        aster_exec_copy_f16_kernel,
        src,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_aster_exec_runs_f16_tail_copy_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([index / 8.0 for index in range(10)], dtype="f16")
    dst = bb.zeros((10,), dtype="f16")

    artifact = bb.compile(
        aster_exec_copy_f16_tail_kernel,
        src,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
        backend="aster_exec",
    )

    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_compile_auto_prefers_aster_exec_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([float(index) for index in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_copy_kernel,
        src,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_compile_auto_prefers_aster_exec_for_mfma_gemm_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_mfma_bf16_gemm_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_bf16_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_mfma_fp8_gemm_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    if target_arch != "gfx942":
        pytest.skip("aster_exec fp8 MFMA currently supports gfx942 only")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[0x40 for _ in range(32)] for _ in range(16)], dtype="fp8")
    b = bb.tensor([[0x40 for _ in range(16)] for _ in range(32)], dtype="fp8")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_fp8_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[32.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_mfma_bf8_gemm_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    if target_arch != "gfx942":
        pytest.skip("aster_exec bf8 MFMA currently supports gfx942 only")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[0x40 for _ in range(32)] for _ in range(16)], dtype="bf8")
    b = bb.tensor([[0x40 for _ in range(16)] for _ in range(32)], dtype="bf8")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_bf8_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[32.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_mfma_fp8_bf8_gemm_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    if target_arch != "gfx942":
        pytest.skip("aster_exec fp8/bf8 MFMA currently supports gfx942 only")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[0x40 for _ in range(32)] for _ in range(16)], dtype="fp8")
    b = bb.tensor([[0x40 for _ in range(16)] for _ in range(32)], dtype="bf8")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_fp8_bf8_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[32.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_mfma_bf8_fp8_gemm_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    if target_arch != "gfx942":
        pytest.skip("aster_exec bf8/fp8 MFMA currently supports gfx942 only")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[0x40 for _ in range(32)] for _ in range(16)], dtype="bf8")
    b = bb.tensor([[0x40 for _ in range(16)] for _ in range(32)], dtype="fp8")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_mfma_bf8_fp8_gemm_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[32.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_fragment_mfma_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_fragment_mfma_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_fragment_mfma_bf16_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_fragment_mfma_bf16_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_tcgen05_fragment_mfma_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="f16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_tcgen05_fragment_mfma_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_warp_fragment_mfma_bf16_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    a = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    b = bb.tensor([[1.0 for _ in range(16)] for _ in range(16)], dtype="bf16")
    c = bb.zeros((16, 16), dtype="f32")

    artifact = bb.compile(
        aster_exec_warp_fragment_mfma_bf16_kernel,
        a,
        b,
        c,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(a, b, c)
    assert c.tolist() == [[16.0 for _ in range(16)] for _ in range(16)]


def test_compile_auto_prefers_aster_exec_for_copy_tail_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([float(index) for index in range(10)], dtype="f32")
    dst = bb.zeros((10,), dtype="f32")

    artifact = bb.compile(
        aster_exec_copy_tail_kernel,
        src,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_compile_auto_prefers_aster_exec_for_add_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([float(index) for index in range(16)], dtype="f32")
    other = bb.tensor([1.0 for _ in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_add_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [left + right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


def test_compile_auto_prefers_aster_exec_for_2d_copy_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([[float(4 * row + col) for col in range(4)] for row in range(4)], dtype="f32")
    dst = bb.zeros((4, 4), dtype="f32")

    artifact = bb.compile(
        aster_exec_copy_2d_kernel,
        src,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_compile_auto_prefers_aster_exec_for_2d_add_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([[float(4 * row + col) for col in range(4)] for row in range(4)], dtype="f32")
    other = bb.tensor([[1.0 for _ in range(4)] for _ in range(4)], dtype="f32")
    dst = bb.zeros((4, 4), dtype="f32")

    artifact = bb.compile(
        aster_exec_add_2d_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [[left + right for left, right in zip(src_row, other_row, strict=True)] for src_row, other_row in zip(src.tolist(), other.tolist(), strict=True)]


def test_compile_auto_prefers_aster_exec_for_2d_f16_copy_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([[float(4 * row + col) / 8.0 for col in range(4)] for row in range(4)], dtype="f16")
    dst = bb.zeros((4, 4), dtype="f16")

    artifact = bb.compile(
        aster_exec_copy_2d_f16_kernel,
        src,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_compile_auto_prefers_aster_exec_for_2d_f16_tail_copy_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([[float(5 * row + col) / 8.0 for col in range(5)] for row in range(2)], dtype="f16")
    dst = bb.zeros((2, 5), dtype="f16")

    artifact = bb.compile(
        aster_exec_copy_2d_f16_tail_kernel,
        src,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, dst)
    assert dst.tolist() == src.tolist()


@pytest.mark.parametrize("op_name,kernel,other_matrix", F32_2D_BINARY_CASES[1:])
def test_compile_auto_prefers_aster_exec_for_other_2d_f32_binary_on_amd_hardware(
    op_name: str,
    kernel,
    other_matrix: list[list[float]],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(_matrix_f32(), dtype="f32")
    other = bb.tensor(other_matrix, dtype="f32")
    dst = bb.zeros((4, 4), dtype="f32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == _binary_nested(src.tolist(), other.tolist(), op_name)


@pytest.mark.parametrize("op_name,kernel,other_matrix", I32_2D_BINARY_CASES)
def test_compile_auto_prefers_aster_exec_for_2d_i32_binary_on_amd_hardware(
    op_name: str,
    kernel,
    other_matrix: list[list[int]],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(_matrix_i32(), dtype="i32")
    other = bb.tensor(other_matrix, dtype="i32")
    dst = bb.zeros((4, 4), dtype="i32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == _binary_nested(src.tolist(), other.tolist(), op_name)


def test_compile_auto_prefers_aster_exec_for_sub_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([float(index) for index in range(16)], dtype="f32")
    other = bb.tensor([1.0 for _ in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_sub_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [left - right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


def test_compile_auto_prefers_aster_exec_for_mul_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([float(index) for index in range(16)], dtype="f32")
    other = bb.tensor([1.5 for _ in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_mul_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [left * right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


def test_compile_does_not_auto_prefer_aster_exec_for_div_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([float(index + 2) for index in range(16)], dtype="f32")
    other = bb.tensor([2.0 for _ in range(16)], dtype="f32")
    dst = bb.zeros((16,), dtype="f32")

    artifact = bb.compile(
        aster_exec_div_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name != "aster_exec"


def test_compile_auto_prefers_aster_exec_for_f16_copy_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([index / 8.0 for index in range(16)], dtype="f16")
    dst = bb.zeros((16,), dtype="f16")

    artifact = bb.compile(
        aster_exec_copy_f16_kernel,
        src,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_compile_auto_prefers_aster_exec_for_f16_tail_copy_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([index / 8.0 for index in range(10)], dtype="f16")
    dst = bb.zeros((10,), dtype="f16")

    artifact = bb.compile(
        aster_exec_copy_f16_tail_kernel,
        src,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_compile_auto_prefers_aster_exec_for_i32_copy_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(list(range(16)), dtype="i32")
    dst = bb.zeros((16,), dtype="i32")

    artifact = bb.compile(
        aster_exec_copy_i32_kernel,
        src,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, dst)
    assert dst.tolist() == src.tolist()


def test_compile_auto_prefers_aster_exec_for_i32_add_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(list(range(16)), dtype="i32")
    other = bb.tensor([2 for _ in range(16)], dtype="i32")
    dst = bb.zeros((16,), dtype="i32")

    artifact = bb.compile(
        aster_exec_add_i32_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [left + right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


def test_compile_auto_prefers_aster_exec_for_add_tail_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([[float(5 * row + col) for col in range(5)] for row in range(2)], dtype="f32")
    other = bb.tensor([[1.0 for _ in range(5)] for _ in range(2)], dtype="f32")
    dst = bb.zeros((2, 5), dtype="f32")

    artifact = bb.compile(
        aster_exec_add_tail_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == _binary_nested(src.tolist(), other.tolist(), "add")


def test_compile_auto_prefers_aster_exec_for_i32_add_tail_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(list(range(10)), dtype="i32")
    other = bb.tensor([2 for _ in range(10)], dtype="i32")
    dst = bb.zeros((10,), dtype="i32")

    artifact = bb.compile(
        aster_exec_add_i32_tail_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [left + right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


@pytest.mark.parametrize("op_name,kernel,other_values", F32_TAIL_BINARY_CASES)
def test_compile_auto_prefers_aster_exec_for_other_tail_f32_binary_on_amd_hardware(
    op_name: str,
    kernel,
    other_values: list[list[float]],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(_tail_matrix_f32(), dtype="f32")
    other = bb.tensor(other_values, dtype="f32")
    dst = bb.zeros((2, 5), dtype="f32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == _binary_nested(src.tolist(), other.tolist(), op_name)


@pytest.mark.parametrize("op_name,kernel,other_values", I32_TAIL_BINARY_CASES)
def test_compile_auto_prefers_aster_exec_for_other_tail_i32_binary_on_amd_hardware(
    op_name: str,
    kernel,
    other_values: list[int],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(list(range(10)), dtype="i32")
    other = bb.tensor(other_values, dtype="i32")
    dst = bb.zeros((10,), dtype="i32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    if op_name == "sub":
        expected = [left - right for left, right in zip(src.tolist(), other.tolist(), strict=True)]
    elif op_name == "mul":
        expected = [left * right for left, right in zip(src.tolist(), other.tolist(), strict=True)]
    else:
        expected = [left // right for left, right in zip(src.tolist(), other.tolist(), strict=True)]
    assert dst.tolist() == expected


@pytest.mark.parametrize("op_name,kernel,other_values", F32_BROADCAST_BINARY_CASES)
def test_compile_auto_prefers_aster_exec_for_f32_broadcast_binary_on_amd_hardware(
    op_name: str,
    kernel,
    other_values: list[float],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([float(index) for index in range(10)], dtype="f32")
    other = bb.tensor(other_values, dtype="f32")
    dst = bb.zeros((10,), dtype="f32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    scalar = other.tolist()[0]
    if op_name == "add":
        expected = [left + scalar for left in src.tolist()]
    elif op_name == "sub":
        expected = [left - scalar for left in src.tolist()]
    elif op_name == "mul":
        expected = [left * scalar for left in src.tolist()]
    else:
        expected = [left / scalar for left in src.tolist()]
    assert dst.tolist() == expected


@pytest.mark.parametrize("op_name,kernel,other_values", I32_BROADCAST_BINARY_CASES)
def test_compile_auto_prefers_aster_exec_for_i32_broadcast_binary_on_amd_hardware(
    op_name: str,
    kernel,
    other_values: list[int],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(list(range(10)), dtype="i32")
    other = bb.tensor(other_values, dtype="i32")
    dst = bb.zeros((10,), dtype="i32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    scalar = other.tolist()[0]
    if op_name == "add":
        expected = [left + scalar for left in src.tolist()]
    elif op_name == "sub":
        expected = [left - scalar for left in src.tolist()]
    elif op_name == "mul":
        expected = [left * scalar for left in src.tolist()]
    else:
        expected = [left // scalar for left in src.tolist()]
    assert dst.tolist() == expected


@pytest.mark.parametrize("op_name,kernel,src_values", F32_LHS_BROADCAST_BINARY_CASES)
def test_compile_auto_prefers_aster_exec_for_f32_lhs_broadcast_binary_on_amd_hardware(
    op_name: str,
    kernel,
    src_values: list[float],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(src_values, dtype="f32")
    other = bb.tensor([float(index + 1) for index in range(10)], dtype="f32")
    dst = bb.zeros((10,), dtype="f32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    scalar = src.tolist()[0]
    if op_name == "sub":
        expected = [scalar - right for right in other.tolist()]
    else:
        expected = [scalar / right for right in other.tolist()]
    assert dst.tolist() == expected


@pytest.mark.parametrize("op_name,kernel,src_values", I32_LHS_BROADCAST_BINARY_CASES)
def test_compile_auto_prefers_aster_exec_for_i32_lhs_broadcast_binary_on_amd_hardware(
    op_name: str,
    kernel,
    src_values: list[int],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(src_values, dtype="i32")
    other = bb.tensor([index + 1 for index in range(10)], dtype="i32")
    dst = bb.zeros((10,), dtype="i32")

    artifact = bb.compile(
        kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    scalar = src.tolist()[0]
    if op_name == "sub":
        expected = [scalar - right for right in other.tolist()]
    else:
        expected = [scalar // right for right in other.tolist()]
    assert dst.tolist() == expected


@pytest.mark.parametrize("op_name,kernel,scalar_values", F32_2D_BROADCAST_CASES)
def test_compile_auto_prefers_aster_exec_for_2d_f32_broadcast_on_amd_hardware(
    op_name: str,
    kernel,
    scalar_values: list[list[float]],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    dense = bb.tensor(_tail_matrix_f32(), dtype="f32")
    scalar = bb.tensor(scalar_values, dtype="f32")
    dst = bb.zeros((2, 5), dtype="f32")

    if op_name == "add":
        artifact = bb.compile(kernel, dense, scalar, dst, target=bb.AMDTarget(arch=target_arch), cache_dir=tmp_path / "cache")
        artifact(dense, scalar, dst)
        expected = _binary_nested(dense.tolist(), [[scalar.tolist()[0][0] for _ in range(5)] for _ in range(2)], "add")
    else:
        artifact = bb.compile(kernel, scalar, dense, dst, target=bb.AMDTarget(arch=target_arch), cache_dir=tmp_path / "cache")
        artifact(scalar, dense, dst)
        scalar_value = scalar.tolist()[0][0]
        expected = [[scalar_value - right for right in row] for row in dense.tolist()]

    assert artifact.backend_name == "aster_exec"
    assert dst.tolist() == expected


@pytest.mark.parametrize("op_name,kernel,scalar_values", I32_2D_BROADCAST_CASES)
def test_compile_auto_prefers_aster_exec_for_2d_i32_broadcast_on_amd_hardware(
    op_name: str,
    kernel,
    scalar_values: list[list[int]],
    tmp_path: Path,
) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    dense = bb.tensor([[5 * row + col for col in range(5)] for row in range(2)], dtype="i32")
    scalar = bb.tensor(scalar_values, dtype="i32")
    dst = bb.zeros((2, 5), dtype="i32")

    if op_name == "add":
        artifact = bb.compile(kernel, dense, scalar, dst, target=bb.AMDTarget(arch=target_arch), cache_dir=tmp_path / "cache")
        artifact(dense, scalar, dst)
        expected = _binary_nested(dense.tolist(), [[scalar.tolist()[0][0] for _ in range(5)] for _ in range(2)], "add")
    else:
        artifact = bb.compile(kernel, scalar, dense, dst, target=bb.AMDTarget(arch=target_arch), cache_dir=tmp_path / "cache")
        artifact(scalar, dense, dst)
        scalar_value = scalar.tolist()[0][0]
        expected = [[scalar_value - right for right in row] for row in dense.tolist()]

    assert artifact.backend_name == "aster_exec"
    assert dst.tolist() == expected


def test_compile_auto_prefers_aster_exec_for_i32_sub_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(list(range(16)), dtype="i32")
    other = bb.tensor([2 for _ in range(16)], dtype="i32")
    dst = bb.zeros((16,), dtype="i32")

    artifact = bb.compile(
        aster_exec_sub_i32_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [left - right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


def test_compile_auto_prefers_aster_exec_for_i32_mul_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor(list(range(16)), dtype="i32")
    other = bb.tensor([3 for _ in range(16)], dtype="i32")
    dst = bb.zeros((16,), dtype="i32")

    artifact = bb.compile(
        aster_exec_mul_i32_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name == "aster_exec"
    artifact(src, other, dst)
    assert dst.tolist() == [left * right for left, right in zip(src.tolist(), other.tolist(), strict=True)]


def test_compile_does_not_auto_prefer_aster_exec_for_i32_div_on_amd_hardware(tmp_path: Path) -> None:
    if os.environ.get("BAYBRIDGE_RUN_EXEC_TESTS") != "1":
        pytest.skip("set BAYBRIDGE_RUN_EXEC_TESTS=1 to exercise ASTER exec on hardware")
    if "BAYBRIDGE_ASTER_ROOT" not in os.environ:
        pytest.skip("set BAYBRIDGE_ASTER_ROOT to an ASTER source checkout")
    target_arch = os.environ.get("BAYBRIDGE_EXEC_ARCH", "gfx942")
    backend = AsterExecBackend()
    if not backend.available(bb.AMDTarget(arch=target_arch)):
        pytest.skip(f"aster_exec is not ready for target {target_arch}")

    src = bb.tensor([2 * (index + 1) for index in range(16)], dtype="i32")
    other = bb.tensor([2 for _ in range(16)], dtype="i32")
    dst = bb.zeros((16,), dtype="i32")

    artifact = bb.compile(
        aster_exec_div_i32_kernel,
        src,
        other,
        dst,
        target=bb.AMDTarget(arch=target_arch),
        cache_dir=tmp_path / "cache",
    )

    assert artifact.backend_name != "aster_exec"
